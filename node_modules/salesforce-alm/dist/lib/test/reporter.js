...
const CLASS_LOCATION_REGEX = new RegExp('Class(?:\\.([\\w]*))?\\.([\\w]*)\\.([\\w]*): line ([0-9]*), column ([0-9]*)');
/**
...
class HumanReporter extends Reporter {
            ...
            },
            {
                ...
                format: (msg, row) => {
                    if (util.isString(msg)) {
                        ...
                        return `${this.logger.color.red(msg)}${!_.isEmpty(row.StackTrace) ? `\n${row.StackTrace}` : ''}`;
                    }
                    return '';
                }
            },
    ...
    logFailuresTable(testResults) {
        if (!testResults.tests) {
            return;
        }
        const failures = testResults.tests.filter(test => util.isString(test.Message) && !_.isEmpty(test.StackTrace));
            ...
            this.logTable('Failures', failures, [
                { key: 'FullName', label: 'FullName' },
                {
                    ...
                    format: (msg, row) => {
                        ...
                        return `${this.logger.color.red(msg)}\n${sourceLinks.join('\n')}`;
                    }
                }
            ]);
        }
