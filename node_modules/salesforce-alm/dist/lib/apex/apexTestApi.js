...
const reporter_1 = require("../test/reporter");
const TestResults = require("../test/testResults");
const TEST_LEVELS = {
 ...
 */
class ApexHumanReporter extends reporter_1.HumanReporter {
    constructor(testApi) {
 ...
 */
class ApexTapReporter extends reporter_1.TapReporter {
    constructor(testApi) {
...
class ApexJUnitReporter extends reporter_1.JUnitReporter {
    constructor(testApi) {
        super(testApi.logger);
    }
}
class ApexJsonReporter extends reporter_1.JsonReporter {
    constructor(testApi) {
        super(testApi.logger);
    }
}
...
class ApexTestApi {
    ...
    constructor(org) {
        ...
        this.reporter = new reporter_1.NoOpReporter(this.logger);
    ...
    initialize(options = {}) {
        ...
        if (this.resultformat || this.wait) {
            ...
            let reporter = exports.ReporterTypes[this.resultformat];
            if (!reporter) {
                return BBPromise.reject(almError('apexTestApiInvalidReporter', Object.keys(exports.ReporterTypes).join(',')));
            }
            ...
            if (this.json) {
                ...
                reporter = exports.ReporterTypes[this.resultformat];
            }
        ...
        return this.setupOutputDirectory().then(() => {
            if (this.hasOutputDirectory() &&
                !this.reporter.isType(reporter_1.NoOpReporter) &&
                // json results written in logTestArtifacts
                !this.reporter.isType(reporter_1.JsonReporter)) {
    ...
    runTests() {
        ...
        if (this.synchronous && (this.suitenames.length > 0 || this.classnames.length > 1)) {
            ...
            return BBPromise.reject(almError({
    ...
    async queryApexCodeCoverage(testClasses = [], queryCharacterLimit = 20000) {
        ...
        if (process.env.SFDX_IMPROVED_CODE_COVERAGE === 'true' && testClasses.length > 0) {
            ...
            const conditions = testClasses.map(testClass => util.format(conditionTemplate, testClass.Name, testClass.NamespacePrefix || ''));
    ...
    logTestArtifacts() {
        ...
        if (this.hasOutputDirectory()) {
            return BBPromise.resolve()
                ...
                const junit = {
                    format: 'junit',
                    file: path.join(this.outputdir, this.testrunid
                ...
                const json = {
                    format: 'json',
                    file: path.join(this.outputdir, this.testrunid ? `${TEST_RESULT_FILE_PREFIX}-${this.testrunid}.json` : `${TEST_RESULT_FILE_PREFIX}.json`)
                };
    ...
    retrieveApexCodeCoverage() {
        return this.org
            ...
            const testClasses = _.uniqWith(this.testResults.tests.map(t => t.ApexClass), (c1, c2) => {
...
exports.ApexTestApi = ApexTestApi;
ApexTestApi.TEST_LEVELS = TEST_LEVELS;
ApexTestApi.ApexHumanReporter = ApexHumanReporter;
