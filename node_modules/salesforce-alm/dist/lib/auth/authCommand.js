...
const webAuthListener_1 = require("./webAuthListener");
...
const handleDemoModePrompt = function (username) {
    if (this.noprompt) {
        ...
        return cli_ux_1.default.prompt(promptMessage);
    }
};
/**
...
const _doSfdxUrlAuth = function (force, context, orgApi) {
    ...
    return srcDevUtil
        ...
        }, orgApi, !!type, handleDemoModePrompt.bind(context));
    });
};
const _doJwtAuth = function (force, context, orgApi, loginUrl) {
    ...
    const oauthConfig = {
        ...
        privateKeyFile: path_1.resolve(context.jwtkeyfile)
    };
...
class AuthCommand {
    constructor(force) {
        ...
        this.force = _.isNil(force) ? new Force(new configApi_1.Config()) : force;
    ...
    validate(context) {
        ...
        const file = context.flags.jwtkeyfile ? path_1.resolve(context.flags.jwtkeyfile) : undefined;
 ...
 * @returns {BBPromise.<Error>}
 */
AuthCommand.doNewAuth = function (org, context, isJwt, isSfdxUrl, force, stdinValues) {
    let promise;
    if (context.instanceurl) {
        ...
        promise = core_1.ConfigAggregator.create().then(aggregator => {
    ...
    if (isSfdxUrl) {
        ...
        promise = promise.then(loginUrl => doAuth(force, clientSecret, context, org, loginUrl, AuthCommand.getOpen()).catch(err => {
