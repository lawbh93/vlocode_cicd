...
class SandboxOrg extends events_1.EventEmitter {
    ...
    async writeAuthFile(sandboxProcessObj, sandboxRes) {
        ...
        if (sandboxRes.authUserName) {
            let authFields = await this.getAuthInfoFields();
            ...
            await authInfo.save();
            let sandboxOrg = await core_1.Org.create({ aliasOrUsername: authInfo.getUsername() });
    ...
    async validateSandboxCompleteAndGetAuthenticationInfo(sandboxProcessObj) {
        ...
        if (endDate) {
            try {
                ...
                const config = new ConfigApi.Config();
                let authFields = await this.getAuthInfoFields();
                let sandboxReq = new sandboxOrgApi_1.SandboxUserAuthRequest();
    ...
    async pollStatusAndAuth(sandboxProcessObj, retries, shouldPoll) {
        ...
        let response = await this.validateSandboxCompleteAndGetAuthenticationInfo(sandboxProcessObj);
        if (response) {
            try {
        ...
        if (!pollFinished) {
            if (retries > 0) {
                ...
                let polledSandboxProcessObj = await this.api.querySandboxProcessById(sandboxProcessObj.Id);
    ...
    async deleteSandbox(sandboxOrgId) {
        ...
        let sandboxProcessObject = await this.api.querySandboxProcessBySandboxOrgId(shortId);
