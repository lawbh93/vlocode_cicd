...
class ShapeRepCreateCommand extends varargsCommand_1.default {
    ...
    async _generateScratchDefFile(context, shapeId) {
    ...
    async _retrieveShapeRecord(shapeId) {
        const logger = await this.getLogger();
    ...
    _makeShapeFileName(fileNameParameter, shapeId, orgId) {
        let workingFileName = fileNameParameter;
    ...
    async _makeConfigPath() {
        const project = await core_1.SfdxProject.resolve();
        const projectPath = project.getPath();
        return path.join(projectPath, '/config');
    }
    async _makeShapeFilePathName(fileNameParameter) {
        const workingFileName = path.join(await this._makeConfigPath(), fileNameParameter);
        return workingFileName;
    }
    async _writeJsonFile(fileNameAndPath, jsonContent) {
        const logger = await this.getLogger();
