...
class User {
    ...
    async create(fields = {}) {
        ...
        const body = JSON.stringify({
            ...
            nickName: username.substring(0, 40),
            entitlements: [
                {
                    value: profileid
                }
        ...
        await this.force._getConnection(this.org, this.org.config).then(async (conn) => {
            ...
            return conn._transport
                ...
                if (response.statusCode === 201) {
                    ...
                    const errMessage = _.get(respBody, 'Errors[0].description') || _.get(respBody, '[0].message') || 'Unknown Error';
    ...
    async assignPermissionSets(permsetNames) {
        if (!this.isCreated) {
            throw almError({ keyName: 'userNotCreated', bundle: 'user_create' }, 'assigning permission sets.');
        }
        this._permissionSetAssignments = await Promise.all(permsetNames.map(permsetName => new permissionSetAssignment_1.default(this.org).create(this, permsetName)));
     ...
     */
    static async fetchUsers(org, usernames, fields = ['id', 'username']) {
        // Build the user query
        const condition = usernames.length === 1 ? `username='${usernames[0]}'` : `(username IN (${singleQuoteJoin(usernames)}))`;
        ...
        if (usernames.length !== userRecords.length) {
            const usersNotFound = _.difference(usernames, _.map(userRecords, 'Username'));
            const errorConfig = usersNotFound.length === 1 ? ['userNotFound', usersNotFound[0]] : ['usersNotFound', usersNotFound.join()];
...
User.isValidPassword = new RegExp(`^(?=.*[${LOWER}])(?=.*[${UPPER}])(?=.*[${NUMBERS}])(?=.*[${SYMBOLS.join('\\')}]).+$`);
