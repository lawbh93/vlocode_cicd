...
class SubscriberPackageVersionQuery {
     ...
     */
    constructor(org, force, logger, allPackageVersionId, installationKey) {
     ...
     */
    async query(retries, pollInterval) {
        while (retries-- >= 0) {
            ...
            if (queryResult == null) {
                ...
                if (queryResult.records && queryResult.records.length > 0) {
                    ...
                    if (this.installationValidationStatus != 'PACKAGE_UNAVAILABLE' &&
    ...
    _populateObject(queryResultRecord) {
    ...
    static isErrorPackageNotAvailable(err) {
        return err.name === 'UNKNOWN_EXCEPTION' || err.name === 'PACKAGE_UNAVAILABLE';
    }
}
SubscriberPackageVersionQuery.SELECT = 'SELECT Package2ContainerOptions, SubscriberPackageId, InstallValidationStatus, ' +
    'RemoteSiteSettings, CspTrustedSites FROM SubscriberPackageVersion ';
module.exports = {
    SubscriberPackageVersionQuery,
    async getSubscriberPackageVersionQuery(org, force, logger, allPackageVersionId, installationKey, retries, pollInterval) {
        ...
        return pvcrApi.byId(id).then(async (results) => {
            if (this._isStatusEqualTo(results, [STATUS_SUCCESS, STATUS_ERROR])) {
                ...
                if (retries > 0) {
                    ...
                    logger.log(`Request in progress. Sleeping ${interval} seconds. Will wait a total of ${interval *
                        retries} more seconds before timing out. Current Status='${this.convertCamelCaseStringToSentence(currentStatus)}'`);
                    return BBPromise.delay(interval * 1000).then(() => this.pollForStatus(context, id, retries - 1, packageId, logger, withProject, force, org));
                }
                else {
                    // Timed out
                }
