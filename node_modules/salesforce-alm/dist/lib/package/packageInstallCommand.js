...
class PackageInstallCommand {
    ...
    async execute(context) {
        ...
        let spv = await pkgUtils.getSubscriberPackageVersionQuery(this.org, this.force, this.logger, this.allPackageVersionId, this.installationKey, publishWaitRetries, this.replicationPollIntervalMillis);
        ...
        if (spv.trustedSites && spv.trustedSites.length > 0) {
            const accepted = await this._prompt(context.flags.noprompt, messages.getMessage('promptRss', spv.trustedSites.join('\n'), 'package_install'));
            if (accepted) {
