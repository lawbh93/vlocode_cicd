        ...
        return BBPromise.resolve().then(() => this._validateDependencyValues(dependency).then(() => {
            ...
            return this._resolveBuildNumber(versionNumber, dependency.packageId, branch).then(queryResult => {
                ...
                if (!records || records.length === 0 || records[0].expr0 == null) {
                    throw new Error(`No version number was found in Dev Hub for package id ${dependency.packageId} and branch ${branch} and version number ${versionNumber.join(pkgUtils.VERSION_NUMBER_SEP)}`);
                }
                ...
                return this.force.toolingQuery(this.org, query).then(pkgVerQueryResult => {
                    ...
                    if (!subRecords || subRecords.length !== 1) {
                        throw new Error(`No version number was found in Dev Hub for package id ${dependency.packageId} and branch ${branch} and version number ${versionNumber.join(pkgUtils.VERSION_NUMBER_SEP)} that resolved to build number ${buildNumber}`);
                    }
