...
class AggregateSourceElement {
    ...
    markForDelete() {
        ...
        allPaths.forEach(deletedPath => {
            ...
            const deletedWorkspaceElement = new workspaceElement_1.WorkspaceElement(deletedMetadataType.getMetadataName(), fullNameForPath, deletedPath, sourceState.DELETED, deleteSupported);
     ...
     * @returns {Array[]}
     */
    commit(manifest, unsupportedMimeTypes, forceoverwrite = false) {
        let newPaths = [];
        let updatedPaths = [];
        let deletedPaths = [];
        let dupPaths = [];
    ...
    commitMetadata(newPaths, updatedPaths, deletedPaths, dupPaths, manifest, forceoverwrite) {
        const [newMetaPaths, updatedMetaPaths, deletedMetaPaths, dupMetaPaths] = this.decomposeMetadata(this.retrievedMetadataPath, this.getMetadataFilePath(), manifest, forceoverwrite);
        newPaths.push(...newMetaPaths);
        updatedPaths.push(...updatedMetaPaths);
        deletedPaths.push(...deletedMetaPaths);
        dupPaths.push(...dupMetaPaths);
    }
    commitContent(newPaths, updatedPaths, deletedPaths, dupPaths, unsupportedMimeTypes, forceoverwrite) {
        const [newContentPaths, updatedContentPaths, deletedContentPaths, dupContentPaths] = this.contentStrategy.saveContent(this.getMetadataFilePath(), this.retrievedContentPaths, this.retrievedMetadataPath, this.isDuplicate, unsupportedMimeTypes, forceoverwrite);
    ...
    addEmptyFolder() {
        if (this.metadataType.isFolderType()) {
            ...
            if (folderPath) {
                this.addWorkspaceElement(new workspaceElement_1.WorkspaceElement(this.getMetadataName(), this.aggregateFullName, folderPath, sourceState.NEW, true));
            }
        }
    }
     ...
     * @returns {Array}
     */
    getFilePathTranslations(mdDir, tmpDir, unsupportedMimeTypes, forceIgnore) {
    ...
    getMetadataPathTranslation(tmpDir, mdDir) {
     ...
     */
    decomposeMetadata(sourceFilePath, metadataFilePath, manifest, forceoverwrite = false) {
