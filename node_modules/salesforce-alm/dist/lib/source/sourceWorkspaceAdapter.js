...
class SourceWorkspaceAdapter extends kit_1.AsyncCreatable {
    ...
    async updateSource(aggregateSourceElements, manifest, checkForDuplicates, unsupportedMimeTypes, forceoverwrite = false) {
        ...
        for (let sourceElement of aggregateSourceElements.getAllSourceElements()) {
            ...
            const [newPathsForElements, updatedPathsForElements, deletedPathsForElements] = sourceElement.commit(manifest, unsupportedMimeTypes, forceoverwrite);
    ...
    processMdapiFileProperty(changedSourceElements, retrieveRoot, fileProperty, bundleFileProperties) {
        ...
        aggregateMetadataPath = this.sourceLocations.getMetadataPath(aggregateMetadataType.getMetadataName(), aggregateFullName);
        ...
        if (this.forceIgnore.accepts(aggregateMetadataPath)) {
            ...
            const key = aggregateSourceElement_1.AggregateSourceElement.getKeyFromMetadataNameAndFullName(aggregateMetadataType.getMetadataName(), aggregateFullName);
        ...
        if (metadataPath !== undefined) {
            ...
            if (shouldDeleteWorkspaceAggregate) {
                ...
                sourcePathsToDelete.forEach(sourcePathToDelete => {
                    const deletedWorkspaceElement = new workspaceElement_1.WorkspaceElement(sourceMemberMetadataType.getMetadataName(), fullName, sourcePathToDelete, sourceState.DELETED, true);
