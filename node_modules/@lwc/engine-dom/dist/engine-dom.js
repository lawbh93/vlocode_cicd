...
const { filter: ArrayFilter, find: ArrayFind, indexOf: ArrayIndexOf, join: ArrayJoin, map: ArrayMap$1, push: ArrayPush$1, reduce: ArrayReduce, reverse: ArrayReverse, slice: ArraySlice, splice: ArraySplice, unshift: ArrayUnshift, forEach, } = Array.prototype;
...
function createComponentDef(Ctor) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (!runtimeFlags.ENABLE_PREFER_NATIVE_SHADOW) {
      assert.isFalse('preferNativeShadow' in Ctor, `${ctorName || 'Anonymous class'} is an invalid LWC component. \`preferNativeShadow\` is not available in this environment.`);
    }

    if (!isUndefined$1(ctorPreferNativeShadow)) {
...
function updateSyntheticShadowAttributes(vm, template) {
  ...
  if (!isUndefined$1(newStylesheets) && newStylesheets.length !== 0 && renderMode === RenderMode.Shadow) {
...
function getStylesheetsContent(vm, template) {
  ...
  if (!isUndefined$1(stylesheets) && stylesheets.length !== 0) {
    ...
    if (renderMode === RenderMode.Shadow && shadowMode === ShadowMode.Synthetic && !isUndefined$1(stylesheetTokens)) {
