...
exports.compileToFunction = exports.parse = exports.IRAttributeType = void 0;
/*
...
function parse(source, config = {}) {
    const options = config_1.mergeConfig(config, { format: 'module' });
...
function compile(source, config) {
    ...
    try {
        ...
        if (!hasParsingError && parsingResults.root) {
            code = codegen_1.default(parsingResults.root, options);
        }
    }
    catch (error) {
...
function compileToFunction(source) {
    const options = config_1.mergeConfig({}, { format: 'function' });
    const state = new state_1.default(options);
    const parsingResults = parser_1.default(source, state);
    ...
    const code = codegen_1.default(parsingResults.root, options);
